...
export default function EditModal({
  ...
  const handleSaveChanges = () => {
    if (name !== newName)
      editData(`workouts/${currentUser}/${id}/name`, newName);
    if (workout) {
      editedVals.map((exercise: TypeExercise, index: number) => {
        if (exercise.reps !== workout[index]?.reps)
          editData(
            `workouts/${currentUser}/${id}/exercises/${index}/reps`,
            exercise.reps
          );
        if (exercise.sets !== workout[index]?.sets)
          editData(
            `workouts/${currentUser}/${id}/exercises/${index}/sets`,
            exercise.sets
          );
        if (!workout[index])
          editData(
            `workouts/${currentUser}/${id}/exercises/${index}/name`,
            exercise.name
          );
      });
    } else {
            ...
            <VStack>
              ...
              <Center w="90%" position={"absolute"} top={"15%"}>
                ...
                >
                      ...
                      >
                        <GridItem colSpan={1}>
                          <IconButton
                            ...
                            onClick={() =>
                              handleRemoveExerciseFromSelectedExs(
                                exercise.name,
                                index
                              )
                            }
                  ...
                  <FetchFromApi
                    selectedExs={editedVals}
                    setSelectedExs={setEditedVals}
                  />
                </VStack>
              </Center>
            </VStack>
