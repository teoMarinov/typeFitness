...
export default function MakeNewWorkoutModal({ currentUser }: TypeProp) {
  ...
  const [editedVals, setEditedVals] = useState<any>([]);
  ...
  const handleRemoveExerciseFromSelectedExs = (name: string, index: number) => {
    const editedList = editedVals.filter((e: any) => e.name !== name);
    ...
    <>
      ...
      >
        ...
        >
          ...
          <ModalFooter>
            <Button colorScheme="green" mr={3} onClick={handleAddNewWorkout}>
              Save
            </Button>
