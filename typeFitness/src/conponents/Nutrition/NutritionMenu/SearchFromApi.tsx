...
import { Grid, GridItem, Center, Input, Text, Spinner } from "@chakra-ui/react";
...
export default function SearchFromApi({ state, dispatch }: propTypes) {
  const [nameInput, setNameInput] = useState("");
  const [loading, setLoading] = useState(false);

  const inputBoxLength =
    state.name.length > 5 ? `${state.name.length * 9}px` : "48px";
  ...
  useEffect(() => {
    ...
    foodFetcher(nameInput)
      .then((snapshot) => {
        ...
        dispatch({
          type: ACTION.CHANGE_CALORIES,
          payload: snapshot[0].calories,
        });
        dispatch({ type: ACTION.CHANGE_FAT, payload: snapshot[0].fat_total_g });
        dispatch({
          type: ACTION.CHANGE_SATURATED_FAT,
          payload: snapshot[0].fat_saturated_g,
        });
        dispatch({
          type: ACTION.CHANGE_CARBOHYDRATE,
          payload: snapshot[0].carbohydrates_total_g,
        });
        dispatch({ type: ACTION.CHANGE_SUGAR, payload: snapshot[0].sugar_g });
        dispatch({
          type: ACTION.CHANGE_PROTEIN,
          payload: snapshot[0].protein_g,
        });
      })
      .then(() => {
        setLoading(false);
