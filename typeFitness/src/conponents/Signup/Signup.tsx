...
const Signup = () => {
  ...
  const onRegister = () => {
    ...
    if (
      form.handle.length < MIN_HANDLE_LENGTH ||
      form.handle.length > MAX_HANDLE_LENGTH
    ) {
    ...
    if (
      ...
      return setError(
        "Password should be longer than 5 symbols, containing at least one number and one capital letter!"
      );
    }

    checkForPhoneNumber(form.phoneNumber)
    ...
    <>
      ...
      >
        ...
        <Center mt={"10px"} textColor="white">
          ...
          <Link as={RouterLink} to="/log-in" color="blue.400">
            Log In
          </Link>
        </Center>
      </AbsoluteCenter>
